...
namespace QLKTX.GUI
{
    public partial class frmNV_AddRoom : Form
    {
        private readonly KTXDBContext context;
        public frmNV_AddRoom()
        {
        ...
        private void frmNV_AddRoom_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            if (CheckValidate())
            {
                ...
                if (GetSelectedRow(txtRoomID.Text) == -1)
                {
                    ...
                    else if (cmbLoaiPhong.SelectedItem.ToString() == "Phòng 6")
                    {
                        phong.SOLUONG = 6;
                        phong.TRANGTHAI = cmbTrangThaiPhong.SelectedItem.ToString();
                    }
                    else if (cmbLoaiPhong.SelectedItem.ToString() == "Phòng 8")
                    {
                        phong.SOLUONG = 8;
                        phong.TRANGTHAI = cmbTrangThaiPhong.SelectedItem.ToString();
                    }
